...
def replace_init_state {n : ℕ} (prob : PlanningTask n) (s : State' n) : PlanningTask n :=
  PlanningTask.mk prob.varNames prob.actions' s prob.goal'

/-- Cast a path from `prob` to `replace_init_state prob s`. -/
noncomputable def cast_path_to_replace {n : ℕ} (prob : PlanningTask n) (s : State' n) {s1 s2 : State n}
...
noncomputable def cast_path_from_replace {n : ℕ} (prob : PlanningTask n) (s : State' n) {s1 s2 : State n}
    (path : Path (replace_init_state prob s) s1 s2) : Path prob s1 s2 := by
  induction path with
  | empty s => exact Path.empty s
  | cons a s2 ha succ rest ih => exact Path.cons a s2 ha succ ih

lemma cast_path_to_replace_cost {n : ℕ} (prob : PlanningTask n) (s : State' n) {s1 s2 : State n}
...
lemma cast_path_from_replace_cost {n : ℕ} (prob : PlanningTask n) (s : State' n) {s1 s2 : State n}
...
noncomputable def cast_plan_to_replace {n : ℕ} (prob : PlanningTask n) (s : State' n)
...
noncomputable def cast_plan_from_replace {n : ℕ} (prob : PlanningTask n) (s : State' n)
...
lemma replace_init_state_init {n : ℕ} (prob : PlanningTask n) (s : State' n) :
    (replace_init_state prob s).init = convertState s := rfl

/-! ### Perfect heuristic -/

def perfect_heuristic {n : ℕ} (prob : PlanningTask n) (s : State' n) : ℕ∞ :=
  let replace_init := PlanningTask.mk prob.varNames prob.actions' s prob.goal'
...
private lemma planner_none_of_unsolvable {n : ℕ} (prob : PlanningTask n) (v : State' n)
...
lemma perferct_heuristic_is_perfect {n : ℕ} (prob : PlanningTask n):
  ...
  · intro v;
    ...
    cases h : planner ( PlanningTask.mk prob.varNames prob.actions' v prob.goal' ) ( fun _ => 0 ) <;> simp_all +decide [ zero_heur_admissible' ];
    · have := planner_complete ( PlanningTask.mk prob.varNames prob.actions' v prob.goal' ) ( fun _ => 0 ) ( zero_heur_admissible' ( PlanningTask.mk prob.varNames prob.actions' v prob.goal' ) ) h; exact this.false ( cast_plan_to_replace prob v plan ) ;
    · have := planner_optimal (PlanningTask.mk prob.varNames prob.actions' v prob.goal') (fun _ => 0) (zero_heur_admissible _) (by rw [h]; simp) (cast_plan_to_replace prob v plan);
      convert this.le using 1;
      · grind;
      · exact cast_path_to_replace_cost prob v plan.path ▸ rfl;
  · unfold perfect_heuristic;
    intro v hv;
    cases h : planner ( PlanningTask.mk prob.varNames prob.actions' v prob.goal' ) ( fun _ => 0 ) <;> simp_all +decide;
  ...
  · intro v hv; unfold perfect_heuristic; exact (by
    ...
    cases h : planner ( PlanningTask.mk prob.varNames prob.actions' v prob.goal' ) ( fun _ => 0 ) <;> simp +decide [ h ];
    · exact h;
    · exact h.symm ▸ by tauto;);

lemma perfect_heuristic_weak_dominates_admissible {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞):
...
def perfect_heuristic_invariant {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞):=
...
lemma invariant_gives_le {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞)
...
private lemma path_cost_ge_heur_of_invariant {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞)
    ...
    | cons a s2 ha succ path' =>
      ...
      calc h start
        ...
        _ = ((Path.cons a (convertState (successor' a start)) ha succ path').cost : ℕ∞) := by
              simp [Path.cost]; push_cast; ring
lemma goal_aware_of_perfect_heuristic_invariant {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞):
...
lemma admissible_of_perfect_heuristic_invariant {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞):
...
-/
lemma perfect_is_goal_aware {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞)
    (hp : heur_is_perfect prob h) (s : State' n)
    (hsat : satisfies' prob.goal' s = true) : h s = 0 := by
  obtain ⟨ _, _, _ ⟩ := hp;
  obtain ⟨ plan, hplan ⟩ := ‹∀ v : State' n, Nonempty ( Plan prob ( convertState v ) ) → ∃ plan : Plan prob ( convertState v ), ↑plan.path.cost = h v› s ⟨ convertState s, Path.empty _, satisfies'_implies_GoalState prob s hsat ⟩;
  have := ‹heur_admissible prob h› s ⟨ convertState s, Path.empty _, satisfies'_implies_GoalState prob s hsat ⟩ ; simp_all +decide [ Path.cost ] ;
...
lemma Successor_of_applicable' {n : ℕ} (a : Action n) (s : State' n)
  ...
  ext i
  simp only [convertState, successor', Set.mem_setOf_eq, Set.mem_union, Set.mem_diff,
    Action.mem_del, Action.mem_add, BitVec.getElem_or, BitVec.getElem_and, BitVec.getElem_not,
    VarSet'.getElem_toBitVec', Fin.getElem_fin]
  grind

lemma solvable_non_goal_has_applicable {n : ℕ} (prob : PlanningTask n) (s : State' n)
    (hs : Nonempty (Plan prob (convertState s)))
    (hng : satisfies' prob.goal' s = false) :
    (prob.actions'.filter (fun a => applicable' a s)) ≠ [] := by
  by_contra h_empty;
  have h_unsolvable : IsEmpty (Plan prob (convertState s)) := by
    constructor;
...
lemma perfect_le_action_succ {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞)
    (hp : heur_is_perfect prob h) (s : State' n)
  ...
  have := hp.1 s (Plan.mk plan_succ.last (Path.cons a (convertState (successor' a s)) (List.mem_dedup.mpr ha) (Successor_of_applicable' a s happ) plan_succ.path) plan_succ.goal);
  convert this using 1;
  rw [ ← hplan_succ ];
  exact mod_cast add_comm _ _

lemma perfect_achieves_min {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞)
    (hp : heur_is_perfect prob h) (s : State' n)
    (hs : Nonempty (Plan prob (convertState s)))
    (hng : satisfies' prob.goal' s = false) :
  ...
  rcases plan with ⟨ goal, path, hgoal ⟩ ; rcases path with ( _ | ⟨ a, s', ha, succ, path ⟩ ) <;> simp_all +decide [ Path.cost ] ;
  · exact absurd ( GoalState_implies_satisfies' prob s hgoal ) ( by aesop );
  · have h_succ : s' = convertState (successor' a s) := by
      unfold successor';
      have := succ.right; simp_all +decide [ Fin.forall_iff, Set.ext_iff ] ;
    have h_succ_solvable : Nonempty (Plan prob (convertState (successor' a s))) := by
...
lemma perfect_le_action_succ_general {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞)
    (hp : heur_is_perfect prob h) (s : State' n)
    (hs : Nonempty (Plan prob (convertState s)))
    (a : Action n) (ha : a ∈ prob.actions')
    (happ : applicable' a s = true) :
...
lemma perfect_heuristic_has_invariant {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞):
...
def weaker_than_perfect_heuristic_invariant {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞):=
  ∀ s : State' n, Nonempty (Plan prob (convertState s)) →
  if satisfies' prob.goal' s  then h s = 0
  else
  (
...
lemma weak_invariant_gives_le {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞)
  ...
  · constructor <;> intro h;
    ...
    · cases h : List.min? ( List.map ( fun a => ( a.cost : ℕ∞ ) + ‹State' n → ℕ∞› ( successor' a s ) ) ( List.filter ( fun a => applicable' a s ) prob.actions' ) ) <;> simp_all +decide [ List.min?_eq_some_iff ];
      exact le_trans ( by tauto ) ( h.2 _ _ ha happ rfl )

private lemma path_cost_ge_heur_of_weak_invariant {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞)
    (ga : heur_goal_aware prob h)
    (hi : weaker_than_perfect_heuristic_invariant prob h)
    (k : ℕ) {start : State' n} {goal : State n}
    (path : Path prob (convertState start) goal)
  ...
  | succ k ih =>
    ...
    | cons a s2 ha succ path' =>
      ...
      calc h start
        ...
        _ = ((Path.cons a (convertState (successor' a start)) ha succ path').cost : ℕ∞) := by
              simp [Path.cost]; push_cast; ring
lemma admissible_of_weak_perfect_heuristic_invariant {n : ℕ} (prob : PlanningTask n) (h : State' n → ℕ∞)
    (ga : heur_goal_aware prob h) :
...
lemma apply_regressable_achieves_goal {n : ℕ} (a : Action n)
  ...
  · unfold convertVarSet varset'_of_state' regress' state'_of_varset' at hprev; simp_all +decide [ Finset.subset_iff, Set.subset_def ] ;
    grind
...
lemma mem_convertVarSet_varset'_of_state' {n : ℕ} (t : State' n) (i : Fin n) :
    i ∈ convertVarSet (varset'_of_state' t) ↔ t[i.val] := by
  rw [mem_convertVarSet]
  simp [varset'_of_state', mem_toVarSet', List.mem_filter, List.mem_finRange]

/-- The preconditions of an action are contained in the set regressed through any state. -/
lemma pre_subset_convert_regress {n : ℕ} (a : Action n) (s : State' n) :
    a.pre ⊆ convertVarSet (varset'_of_state' (regress' a s)) := by
  intro i hi
...
lemma heur_le_regressable_action_cost {n : ℕ} (prob : PlanningTask n)
    (h : PlanningTask n → State' n → ℕ∞) (g : VarSet' n) (s : State' n)
  ...
  by_cases hsolv : Nonempty (Plan (replace_goal prob g') (convertState s))
      ...
      a.cost + h (replace_goal prob (varset'_of_state' (regress' a (state'_of_varset' g)))) s =
        h (replace_goal prob g) s := by
          ...
          have hcost_eq : h (replace_goal prob (varset'_of_state' (regress' a (state'_of_varset' g)))) s ≤ prefix_path.cost := by
            have := hperf ( varset'_of_state' ( regress' a ( state'_of_varset' g ) ) ) |>.1 s ⟨ s_prev, cast_path_replace_goal prob g ( varset'_of_state' ( regress' a ( state'_of_varset' g ) ) ) prefix_path, hprev ⟩ ; simp_all +decide [ Path.cost ] ;
...
def perfect_heuristic_regression_invariant {n : ℕ} (prob : PlanningTask n) (h : PlanningTask n → State' n → ℕ∞):=
  ∀ s : State' n, ∀ g : VarSet' n,(
    ...
    (IsEmpty (Plan (replace_goal prob g) (convertState s)) → (h (replace_goal prob g) s) = ⊤)
...
private lemma plan_cost_ge_heur_of_regression_aux {n : ℕ} (prob : PlanningTask n)
  ...
  · cases path <;> simp_all +decide [ Path.length ];
  ...
  · have := hinv start g;
    ...
    · exact this.1 ⟨ goal, path, goal_state ⟩ ▸ zero_le _;
    · rcases path with ( _ | ⟨ a, s_mid, ha, succ, rest ⟩ ) <;> simp_all +decide [ Path.length ];
      · exact absurd ( GoalState_implies_satisfies' ( replace_goal prob g ) start goal_state ) ( by aesop );
      · obtain ⟨s_prev, a_last, ha_last, prefix_path, succ_last, heq, hlen'⟩ := Path.cons_to_snoc ha succ rest;
        have hcost_eq : h (replace_goal prob g) start ≤ a_last.cost + h (replace_goal prob (varset'_of_state' (regress' a_last (state'_of_varset' g)))) start := by
          ...
          · exact ⟨ ⟨ goal, Path.cons a s_mid ha succ rest, goal_state ⟩ ⟩;
        ...
        have hcost_eq : h (replace_goal prob (varset'_of_state' (regress' a_last (state'_of_varset' g)))) start ≤ prefix_path.cost := by
          convert ih _ _ ( cast_path_replace_goal prob g ( varset'_of_state' ( regress' a_last ( state'_of_varset' g ) ) ) prefix_path ) _ _ using 1;
...
def weaker_than_perfect_heuristic_regression_invariant {n : ℕ} (prob : PlanningTask n) (h : PlanningTask n → State' n → ℕ∞):=
  ∀ s : State' n, ∀ g : VarSet' n,(
    Nonempty (Plan (replace_goal prob g) (convertState s)) → (
      -- consider any potential goal
      if satisfies' g s then h (replace_goal prob g) s = 0
...
private lemma plan_cost_ge_heur_of_weaker_regression_aux {n : ℕ} (prob : PlanningTask n)
  ...
  · by_cases hsat : satisfies' g start;
    ...
    · rcases path with ( _ | ⟨ a, s_mid, ha, succ, rest ⟩ ) <;> simp_all +decide [ Path.length ];
      ...
      · obtain ⟨s_prev, a_last, ha_last, prefix_path, succ_last, heq, hlen'⟩ := Path.cons_to_snoc ha succ rest;
        have hcost_eq : h (replace_goal prob g) start ≤ a_last.cost + h (replace_goal prob (varset'_of_state' (regress' a_last (state'_of_varset' g)))) start := by
          apply weaker_regression_invariant_le prob h hinv g start ⟨⟨goal, Path.cons a s_mid ha succ rest, goal_state⟩⟩ (by
...
def bellman_heuristic_regression_invariant {n : ℕ} (prob : PlanningTask n) (h : PlanningTask n → State' n → ℕ∞):=
∀ s : State' n, ∀ g : VarSet' n,
-- consider any potential goal
if satisfies' g s then h (replace_goal prob g) s = 0
else
...
private lemma plan_cost_ge_heur_of_bellman_regression_aux {n : ℕ} (prob : PlanningTask n)
  ...
  · by_cases hsat : satisfies' g start;
    · exact goal_aware_of_bellman_regression_invariant prob h hinv g start hsat ▸ zero_le _;
    · rcases path with ( _ | ⟨ a, s_mid, ha, succ, rest ⟩ ) <;> simp_all +decide [ Path.length ];
...
def h_1_heuristic_regression_invariant {n : ℕ} (prob : PlanningTask n) (h : PlanningTask n → State' n → ℕ∞):=
  ∀ s : State' n, ∀ g : VarSet' n,
      -- consider any potential goal
      if satisfies' g s then h (replace_goal prob g) s = 0
      else
...
lemma h_1_implies_bellman_for_singletons {n : ℕ} (prob : PlanningTask n)
  ...
  convert hinv s ( singletonVarSet g ) |> fun h => h |> fun h => ?_;
  split_ifs at h ; simp_all +decide [ singletonVarSet ];
  · unfold toVarSet' at *; simp_all +decide [ VarSet'.toList ] ;
  · exact h a ha hreg

lemma plan_for_conjunction_gives_plan_for_atom {n : ℕ} (prob : PlanningTask n)
    (g : VarSet' n) (g' : Fin n) (hg' : g' ∈ g.toList)
    (s : State' n) (plan : Plan (replace_goal prob g) (convertState s)) :
    Nonempty (Plan (replace_goal prob (singletonVarSet g')) (convertState s)) := by
  refine' ⟨ ⟨ _, _, _ ⟩ ⟩;
  exact plan.last;
...
private lemma goalState_atom_of_goalState_conjunction {n : ℕ} (prob : PlanningTask n)
  ...
  have h_g'_in_rg : convertVarSet (singletonVarSet g') ⊆ convertVarSet rg := by
    ...
    simp [convertVarSet, singletonVarSet, hg'];
    convert hg' using 1;
...
private lemma h_1_singleton_plan_cost_ge_heur_aux {n : ℕ} (prob : PlanningTask n)
    ...
    (path : Path (replace_goal prob (singletonVarSet g_atom)) (convertState start) goal)
    (hlen : path.length ≤ k)
  ...
  · by_cases hsat : satisfies' (singletonVarSet g_atom) start
    · exact goal_aware_of_h_1_regression_invariant prob h hinv (singletonVarSet g_atom) start hsat ▸ zero_le _
    · rcases path with _ | ⟨ a, s_mid, ha, succ, rest ⟩
      ...
      · obtain ⟨s_prev, a_last, ha_last, prefix_path, succ_last, heq, hlen'⟩ := Path.cons_to_snoc ha succ rest
        ...
        have hprefix_len : prefix_path.length ≤ k := by
          have hc : (Path.cons a s_mid ha succ rest).length ≤ k + 1 := hlen
          simp only [Path.length] at hc
          omega
