...
def h_plus {n : ℕ} (prob : PlanningTask n) (s : State' n) : ℕ∞ :=
  let del_relax_prob := PlanningTask.mk prob.varNames (prob.actions'.map delete_relax_action) s prob.goal'
...
private lemma dr_path_transfer_of_actions_eq {n : ℕ} (pt1 pt2 : PlanningTask n)
    (h : pt1.actions' = pt2.actions') {s1 s2 : State n} (p : Path pt1 s1 s2) :
    ∃ q : Path pt2 s1 s2, q.cost = p.cost := by
  induction p with
...
private lemma dr_relax_path {n : ℕ} (prob : PlanningTask n) {s1 s2 : State n}
    (p : Path prob s1 s2) {t1 : State n} (hsub : s1 ⊆ t1) :
    ∃ (t2 : State n), s2 ⊆ t2 ∧
      ∃ (q : Path (delete_relaxation prob) t1 t2), q.cost = p.cost := by
  induction p generalizing t1 with
  | empty t => exact ⟨t1, hsub, Path.empty t1, rfl⟩
  | cons a s_mid ha succ π ih =>
    ...
    exact ⟨t2, ht2, Path.cons (delete_relax_action a) (t1 ∪ a.add) hmem hsucc' q, by
      simp [Path.cost, delete_relax_action, hq]⟩
...
private lemma del_relax_plan_exists_from_path {n : ℕ} (prob : PlanningTask n) (v goal : State' n)
    (goal_in_goals : goal ∈ trans_of_STRIPS_goals prob)
    (path : (trans_of_STRIPS prob).Path v goal) :
    ∃ plan : Plan (PlanningTask.mk prob.varNames (prob.actions'.map delete_relax_action) v prob.goal')
  ...
  set dp := PlanningTask.mk prob.varNames (prob.actions'.map delete_relax_action) v prob.goal'
  obtain ⟨last_del, _, h_last_sat, p, h_p_cost⟩ :=
    del_relax_path_from_walk prob dp rfl rfl v goal v (fun _ h => h) path.val h_goal_sat
  -- Build a plan
  have h_goal_state : (PlanningTask.mk prob.varNames (prob.actions'.map delete_relax_action) v prob.goal').GoalState (convertState last_del) := by
    exact satisfies'_implies_GoalState (PlanningTask.mk prob.varNames (prob.actions'.map delete_relax_action) v prob.goal') last_del h_last_sat
...
lemma h_plus_admissible {n : ℕ} (prob : PlanningTask n) : heur_admissible' prob (h_plus prob) := by
  ...
  case h_1 h_none =>
    ...
    have : PlanningTask.init (PlanningTask.mk prob.varNames (prob.actions'.map delete_relax_action) v prob.goal') = convertState v := rfl
    rw [show Unsolvable _ = IsEmpty (Plan _ (convertState v)) from by unfold Unsolvable UnsolvableState PlanningTask.init; rfl] at unsolvable
  ...
  case h_2 ret h_some =>
    ...
    let dp := PlanningTask.mk prob.varNames (prob.actions'.map delete_relax_action) v prob.goal'
    ...
    exact_mod_cast le_trans h_opt h_del_plan_cost