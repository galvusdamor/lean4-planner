...
lemma admissible'_of_heur_admissible' {n : ℕ} (prob : PlanningTask n) (heur : BitVec n → ℕ∞)
...
lemma heur_ne_top_on_path {n : ℕ} (prob : PlanningTask n) (heur : BitVec n → ℕ∞)
    (admissible : heur_admissible' prob heur)
    {goal : BitVec n} (goal_in_goals : goal ∈ trans_of_STRIPS_goals prob)
    (p : (trans_of_STRIPS prob).Path (state'_of_varset' prob.init') goal) :
...
def planner {n : ℕ} (prob : PlanningTask n) (heur : BitVec n → ℕ∞): Option (PlanningTask.Plan prob prob.init) :=
  ...
  | .some ret =>
    ...
    have sat : satisfies' prob.goal' goal' := by
      ...
      apply (satisfies'_iff prob.goal' goal').mpr
      intro i hi
      exact goal'_in_goals.2 i (by simpa using hi)
    ...
    let plan : PlanningTask.Plan prob prob.init := PlanningTask.Plan.mk (convertState ret.fst) path goal_sat
    Option.some plan

/--admissibility not needed: we only needed that the heuristic is not discarding solvable states -/
lemma planner_complete {n : ℕ} (prob : PlanningTask n) (heur : BitVec n → ℕ∞)
      (admissible : heur_admissible' prob heur):
      planner prob heur = Option.none → PlanningTask.Unsolvable prob := by
  intro ret_none
  unfold PlanningTask.Unsolvable PlanningTask.UnsolvableState
  constructor
  intro plan
  let plan_path : PlanningTask.Path prob prob.init plan.last := plan.path
  unfold PlanningTask.init at plan_path
  have last_dec : DecidablePred (Set.Mem plan.last) := last_dec prob (state'_of_varset' prob.init') plan.last plan_path
  ...
  · expose_names
    ...
    have h_complete := NatGraph.astar_multigoal_is_complete heur (state'_of_varset' prob.init')
...
lemma astar_multigoal_cross_goal_optimal {n : ℕ} (prob : PlanningTask n) (heur : BitVec n → ℕ∞)
    (goals : List (BitVec n))
    ...
    (returned_path : Option.isSome (NatGraph.astar_multigoal (g := trans_of_STRIPS prob)
      heur (state'_of_varset' prob.init') goals)) :
    ∀ goal ∈ goals, ∀ p : (trans_of_STRIPS prob).Path (state'_of_varset' prob.init') goal,
      ((NatGraph.astar_multigoal (g := trans_of_STRIPS prob)
        heur (state'_of_varset' prob.init') goals).get returned_path).2.cost ≤ p.cost := by
  ...
  have h_some : ∃ aug_path, NatGraph.astar
      ...
      (NatGraph.opt_heur heur) (some (state'_of_varset' prob.init')) none = some aug_path := by
    exact Option.isSome_iff_exists.mp
      (NatGraph.astar_multigoal_some_implies_astar_some heur (state'_of_varset' prob.init') goals returned_path)
  ...
  have aug_optimal : aug_path.is_cheapest := by
    have aug_ret : Option.isSome (NatGraph.astar
        (g := (trans_of_STRIPS prob).add_artificial_goal (· ∈ goals))
        (NatGraph.opt_heur heur) (some (state'_of_varset' prob.init')) none) := by
      rw [h_eq]; simp
    have h := NatGraph.astar_is_optimal
      (g := (trans_of_STRIPS prob).add_artificial_goal (· ∈ goals))
      (NatGraph.opt_heur heur) (some (state'_of_varset' prob.init')) none
      (NatGraph.opt_heur_admissible heur admissible) aug_ret
    have h_get : (NatGraph.astar
        (g := (trans_of_STRIPS prob).add_artificial_goal (· ∈ goals))
        (NatGraph.opt_heur heur) (some (state'_of_varset' prob.init')) none).get aug_ret = aug_path := by
      simp [h_eq]
    rw [h_get] at h
    exact h
  ...
  have h2 := NatGraph.astar_multigoal_cost_le_aug
    heur (state'_of_varset' prob.init') goals returned_path aug_path h_eq
  exact le_trans h2 h1

/-- When planner returns some, the underlying A* also returns some. -/
lemma planner_isSome_implies_astar_isSome {n : ℕ} (prob : PlanningTask n) (heur : BitVec n → ℕ∞)
    (ret_plan : (planner prob heur).isSome) :
    Option.isSome (NatGraph.astar_multigoal (g := trans_of_STRIPS prob)
      heur (state'_of_varset' prob.init')
...
lemma planner_path_cost_eq_astar {n : ℕ} (prob : PlanningTask n) (heur : BitVec n → ℕ∞)
    (ret_plan : (planner prob heur).isSome) :
...
lemma plan_cost_ge_astar {n : ℕ} (prob : PlanningTask n) (heur : BitVec n → ℕ∞)
    (admissible : heur_admissible prob heur)
  ...
  obtain ⟨g', hg'⟩ : ∃ g' : BitVec n, convertState g' = plan.last := by
  ...
  obtain ⟨w, hw⟩ : ∃ w : WeightedDiGraph.Walk (G := trans_of_STRIPS prob) (state'_of_varset' prob.init') g', w.cost ≤ (hg' ▸ plan.path).cost := by
    apply strips_path_has_cheaper_walk;
  obtain ⟨p, hp⟩ : ∃ p : (trans_of_STRIPS prob).Path (state'_of_varset' prob.init') g', p.cost ≤ w.cost := by
    exact WeightedDiGraph.Walk.cheaper_path_exists w
  have h_optimal : (NatGraph.astar_multigoal (g := trans_of_STRIPS prob) heur (state'_of_varset' prob.init')
    ((List.finRange (2^n)).filter (fun s => satisfies' prob.goal' s))).get astar_some |>.2.cost ≤ p.cost := by
      ...
      · simp
        ...
        · have gs := plan.goal
          rw [← hg'] at gs
          intro i hi
          exact (satisfies'_iff prob.goal' g').mp
            (GoalState_implies_satisfies' prob g' gs) i (by simpa using hi)
  grind



lemma planner_optimal {n : ℕ} (prob : PlanningTask n) (heur : BitVec n → ℕ∞)
  (admissible : heur_admissible prob heur)
  (ret_plan : (planner prob heur).isSome):
  ∀ plan : PlanningTask.Plan prob prob.init, plan.path.cost ≥ ((planner prob heur).get ret_plan).path.cost := by
  intro plan
